...
def updateDB(db, cursor, image, dense, dense1, outputs):
    ...
    sql = "UPDATE " + coronet_train_table + " set image_ldp_path=%s, dense_ldp=%s, dense_ldp_1=%s, ldp_class=%s where id=%s"
    id_index = 0
    print(sql)
...
x_train, y_train, path_train = genDataset3DWithNames(normal_ldp_train_path, bacterial_ldp_train_path, viral_ldp_train_path, covid_ldp_train_path)
